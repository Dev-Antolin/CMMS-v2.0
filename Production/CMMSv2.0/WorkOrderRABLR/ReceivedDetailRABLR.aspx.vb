...
Partial Class WorkOrder_ReceivedDetailRABLR
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            Me.Session("Click") = "RWORABLR"
            'ddlDesignationTo.Visible = False
            'ddlEscalateTo.Visible = False
            'lbAssignToSrch.Visible = False
            btnSaveAst.Visible = False
            disRecWO()
            disAssesstment()
    ...
    '    WrkNo = txtWorkOrderNo.Text
    '    AuthorName = Me.Session("fname")
    '    EscalName = lblWOSelectEscal.Text
    '    SysCreatorID = Me.Session("res_id")
    '    SysModifierID = Me.Session("res_id")
    ...
    '    If txtAssessment.Text = "" Then
    '        lblWOError.Text = "Please specify your assesstment!"
    '        Exit Sub
    '    End If
    ...
    '        lblWOError.Text = Me.Session("InsertErrorRec")
    '    Else
    '        lblWOError.Text = "Assesstment save successfully!"
    '        btnSaveAst.Enabled = False
    '        btnCloseAst.Enabled = False
    ...
    '    mySqlInsertDetail = "update cmms_wo_detail set escalated_date = now(), escalated_by = '" & AuthorName.Trim & "', escalated_to = '" & EscalName.Trim & "', correctiveaction = '" & Enclose(txtAssessment.Text.Trim) & "', sys_modified = now(), sys_modifier = '" & SysModifierID.Trim & "' " _
    '                      & "where wo_no = '" & WrkNo.Trim & "';" _
    '                      & "insert into cmms_wo_history(wo_no,escalated_date,escalated_by,escalated_to,resolve,correctiveaction,sys_created,sys_modified,sys_creator,sys_modifier)" _
    '                      & "values('" & WrkNo.Trim & "',now(),'" & AuthorName.Trim & "','" & EscalName.Trim & "','N','" & Enclose(txtAssessment.Text.Trim) & "',now(),now(),'" & SysCreatorID.Trim & "','" & SysModifierID.Trim & "');"
    '    con.ConnectionString = Me.Session("strCon")
    '    If con.State = ConnectionState.Closed Then
    '        con.Open()
    '        oTran = con.BeginTransaction()
    ...
    Private Sub disAssesstment()
        ...
        mySQLconcat = "SELECT escalated_by, correctiveAction, date_format(Sys_modified, '%Y/%m/%d') as Sys_modified FROM cmms_wo_history where escalated_by = '" & EscalatedBy.Trim & "' and wo_no = '" & RecWorkOrderNumber.Trim & "';"
    ...
    Private Sub disRecWO()
        ...
        Try
            ...
            If reader.Read() Then
                ...
                lblWOSelectEscal.Text = reader.Item("Author_Name")
    ...
    '    Try
    '        Dim mySql As String
    '        mySql = "select Escalate_Desc from cmms_wo_escalation where escalate_desc like '%LPT%' or escalate_desc like '%RCT%' or escalate_desc like '%DIV%' order by Escalate_Desc;"
    '        ds = Execute_DataSetCMMS(mySql)
    ...
    '            mySql = "select Division from irdivision where Division = 'BOS' and zonecode = 'vismin';"
    '        Else
    '            mySql = "select Division from irdivision where Division = 'BOS' and zonecode = 'luzon';"
    '        End If
    ...
    '    Try
    '        Dim mySql As String
    '        mySql = "select distinct region from cmms_users where region like '%Region%';"
    '        ds = Execute_DataSet(mySql)
    ...
    Protected Sub btnWOOk_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnWOOk.Click
        Response.Redirect("ReceivedWORABLR.aspx")
    End Sub
